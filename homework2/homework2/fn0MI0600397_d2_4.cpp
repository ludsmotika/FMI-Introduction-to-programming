...
int maxElementInJaggedArray(const int* const* matrix, int rows)
{
	if (!matrix)
	{
		return 0;
...
int dfs(const int* const* matrix, int rows, const int* const* longestPathsHistory, int x, int y, int prevValue, int currentLength)
{
...
int longestIncreasingPath(const int* const*  matrix, int rows)
{
	if (!matrix)
	{
		return 0;
